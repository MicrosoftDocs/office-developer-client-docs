...
CreateAppointmentsFromXml uses the [Add](/dotnet/api/system.collections.generic.list-1.add) method of the [List\<T\>](/dotnet/api/system.collections.generic.list-1) class in the [System.Collections.Generic](/dotnet/api/system.collections.generic) namespace to aggregate these AppointmentItem objects. When the method has processed all the appointments in the XML data, it returns the AppointmentItem objects in an array.
