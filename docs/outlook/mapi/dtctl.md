...
| `~` <br/> |Indicates that these characters are not allowed (for example,  `"[~0-9]")`.  <br/>  `\` <br/> Used to quote any of the previous symbols (for example,  `"[\-\\\[\]]"` means -, \, [, and ] characters are allowed).  <br/> |
